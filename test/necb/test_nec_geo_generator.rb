...
class GeoTest < CreateDOEPrototypeBuildingTest
  def create_buildings(drop_value_from_array)
    ...
    #intermediatestep.pop
    #intermediatestep.pop
    intermediatestep.push(intermediatestep[0])
    #raise 'hell
    intermediatestep.drop(drop_value_from_array)
    buildilng_zones = intermediatestep[0..19]
    standard = Standard.build('NECB2011')
    #puts "number of non-wildcard spacetypes : #{spacetypes.size}"
    #determine number of floors

    all_spacetypes = buildilng_zones
